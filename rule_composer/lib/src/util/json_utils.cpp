...
{
    json_utils::json_utils( std::shared_ptr< spdlog::logger > logger_ ) : logger { std::move( logger_ ) }
    {
    }

    json_utils::json_utils( std::string file_path_, std::shared_ptr< spdlog::logger > logger_ ) :
        logger { std::move( logger_ ) },
        default_file_path { std::move( file_path_ ) }
    {
    }
    ...
    void json_utils::save_file( const std::string& file_path_ ) const
    {
        validate_json_path( file_path_ );
    ...
    void json_utils::save_file( ) const
    {
        save_file( default_file_path );
    }

    void json_utils::print( ) const
    {
        if ( document.empty( ) )
        {
    ...
    nlohmann::json* json_utils::navigate( const std::string& path )
    {
    ...
    {
        return std::equal( ext.rbegin( ), ext.rend( ), file_path_.rbegin( ), []( char a, char b ) { return std::tolower( a ) == std::tolower( b ); } );
    }

    nlohmann::json* json_utils::parse_array( const std::string& field_name, nlohmann::json* current ) const
    {
