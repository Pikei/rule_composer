#ifndef RULE_COMPOSER_JSON_UTILS_HPP
#define RULE_COMPOSER_JSON_UTILS_HPP
...
public:
     ...
     */
    explicit json_utils( const logger_t& logger_ );
     ...
     * \param logger_ Logger used for displaying messages in the console
     */
    explicit json_utils( const std::string& file_path_, const logger_t& logger_ );
     ...
     */
    void save_file( const std::string& file_path_ );
     ...
     */
    template < typename T > T get_value( const std::string& path )
    {
        if ( document.empty( ) )
        {
     ...
     * The path to the "param" field value will be "object.list[0].param"
     */
    template < typename T > std::optional< T > get_optional( const std::string& path )
    {
        const auto* node = navigate( path );
...
private:
     ...
     * \return A pointer to the current fragment of the JSON file containing the value indicated by the path, or \code nullptr\endcode if the resource was not
     * found.
     */
    json* parse_array( const std::string& field_name, json* current ) const;
...
#endif // RULE_COMPOSER_JSON_UTILS_HPP
